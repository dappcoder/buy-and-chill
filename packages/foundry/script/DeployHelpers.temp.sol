...
contract ScaffoldETHDeploy is Script {
    ...
    modifier ScaffoldEthDeployerRunner() {
        ...
        _;
        _stopBroadcast();
        // Skip exporting deployments to avoid permission issues
        // exportDeployments();
        
        // Instead, just log the deployments to the console
        logDeployments();
    }
    ...
    // New function to log deployments to console instead of writing to file
    function logDeployments() internal {
        console.log("=== DEPLOYMENT SUMMARY ===");
        uint256 len = deployments.length;
        
        for (uint256 i = 0; i < len; i++) {
            console.log(string.concat(deployments[i].name, ": ", vm.toString(deployments[i].addr)));
        }
        console.log("=========================");
    }
    ...
    function exportDeployments() internal {
        // This function is intentionally disabled to avoid permission issues
        console.log("Skipping deployment export to file due to permission issues");
    }
